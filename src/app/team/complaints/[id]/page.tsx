...
import { getComplaintById, updateComplaintStatus } from "@/lib/store";
...
export default function TeamComplaintDetailPage() {
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <Link href="/team/complaints" className="inline-flex items-center gap-2 text-sm text-bu-blue hover:underline">
      ...
      <Card>
        <CardHeader
          ...
          subtitle={`${complaint.studentName} · Filed ${formatDate(complaint.createdAt)}`}
          action={
        ...
        <CardBody>
          ...
          {complaint.photos.length > 0 && (
            <div className="flex flex-wrap gap-3 mt-4">
              {complaint.photos.map((p, i) => (
                <img key={i} src={p} alt={`Evidence ${i + 1}`} className="w-28 h-28 rounded-xl object-cover border" />
              ))}
            </div>
          )}
          <div className="flex flex-wrap gap-2 mt-6 pt-4 border-t border-slate-100">
            ...
            {complaint.status !== "resolved" && (
              <Button variant="secondary" onClick={() => handleStatus("resolved")}>
                Mark Resolved
              </Button>
            )}
          </div>
      ...
      <Card>
        <CardHeader title="Chat with Student" subtitle="Respond to the student's concerns" />
        <CardBody>
